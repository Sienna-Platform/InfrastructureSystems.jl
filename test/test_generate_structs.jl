@testset "Test generated structs" begin
    ...
    # IS.test_generated_structs pairs the two directory listings positionally, so
    # non-generated files (CLAUDE.md) must be excluded from the compared directory.
    compare_dir = mktempdir()
    for f in readdir(existing_dir)
        if endswith(f, ".jl")
            cp(joinpath(existing_dir, f), joinpath(compare_dir, f))
        end
    end
    @test IS.test_generated_structs(descriptor_file, compare_dir)
end
...
@testset "OpenAPI converter codegen: absent openapi_type is untouched" begin
    # converter-plan Task 1.2's hard invariant: a descriptor entry without `openapi_type`
    # must never gain a converter, even when another entry in the same generation call is
    # annotated (proven below by generating both together, not this struct alone).
    outdir = mktempdir()
    data = [
        Dict{String, Any}(
...
@testset "OpenAPI converter codegen: annotated struct emits both Val methods" begin
    ...
    # Reference: `refs[po.<name>]`, identical in both methods.
    @test occursin("bus = refs[po.bus],", device_body)
    @test occursin("bus = refs[po.bus],", natural_body)
    ...
    # the component's own base_power (S_base per converter-plan §8).
...
@testset "OpenAPI converter codegen: openapi_unit pu override" begin
    ...
    @test_throws IS.DataFormatError IS.generate_structs(
        mktempdir(),
        bad_value;
        print_results = false,
    )
...
@testset "OpenAPI converter codegen: generation-time errors, never partial output" begin
    ...
    # (converter-plan R8: parametric codegen needed before Service types) — refuse rather
    # than emit a converter for the wrong (UnionAll) type.
    parametric_openapi = [
        Dict{String, Any}(
